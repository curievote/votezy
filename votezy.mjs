...
console.log(`Starting Votezy v${version} on @${votebot} - Script By @${authors.join(', @')}`);

// Transfer operation found? Lets see if it is for us!
var process_transfer = function (op) {
    const depositer = op.from;
    const currency = op.amount.split(' ');
    const depositmemo = op.memo;
...
function parseBlock(blockNum) {
    steem.api.getBlock(blockNum, async function (err, block) {
        try {
            ...
            log(`Found Block #${blockNum} at ${block.timestamp}, ${timestamp.fromNow()}, ${moment().diff(timestamp, 'seconds')} seconds by ${block.witness} with ${block.transactions.length} transactions and a size of ${getBinarySize(blockJSON)}`);
            blockNum = blockNum + 1;
            ...
            setTimeout(() => parseBlock(blockNum + 1));
        } catch (e) {
            log(`bailing with ${e}`);
            return bail(e);
        }
